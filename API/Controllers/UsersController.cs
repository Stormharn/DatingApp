...
public class UsersController : BaseAPIController
{
    ...
    {
        var userGenderPrefs = await _uow.UserRepo.GetUserGenderPrefs(User.GetUsername());
        userParams.CurrentUsername = User.GetUsername();
        
        if (string.IsNullOrEmpty(userParams.Gender))
        {
            userParams.Gender = userGenderPrefs.Gender;
        }
        if (string.IsNullOrEmpty(userParams.InterestedIn))
        {
            userParams.InterestedIn = userGenderPrefs.InterestedIn.IsNullOrEmpty() ? "any" : userGenderPrefs.InterestedIn;
        }

        var users = await _uow.UserRepo.GetMembersAsync(userParams);
    ...
    {
        return await _uow.UserRepo.GetMemberAsync(username);
    }
